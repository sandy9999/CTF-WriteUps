...
def decrypt(ctext):
	...
	result = recvall(s)
	s.close()
	#You are not allowed to decrypt! means padding is correct
...
def attack(ciphertext):
	length = 16
	#First block of 16 characters is IV, so iterate from 16th character onwards to 47th character over ciphertext
	for i in range(16, 32):
		#My attack ciphertext is IV + ciphertext till (i-1)th character + (ASCII character just after ith character in ciphertext) + remaining portion of ciphertext
